// ══════════════════════════════════════════════════════════════════
// Z-WEALTH · TRADING WORKER + TELEGRAM WEBHOOK + BOT NOTIFY + EXCHANGE BALANCE v19.2
//
...
async function processUser(sb: SB, cfg: BotConfig): Promise<string[]> {
  ...
  if (entered === 0) {
    ...
    if (lastNotifMemory === 0) {
      // Cold start: coba baca dari DB — pakai kolom yang sudah pasti ada
      try {
        const { data: riskRow } = await sb
          ...
          .eq("user_id", userId)
          .maybeSingle();
        // Jika ada data apapun di bot_risk_state → set lastNotif ke sekarang
        // (anggap notif baru saja dikirim agar tidak spam saat cold start)
        if (riskRow) {
          lastNotif = now; // suppress notif sampai interval berikutnya
