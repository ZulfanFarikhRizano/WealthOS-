// ══════════════════════════════════════════════════════════════════
// Z-WEALTH · TRADING WORKER + TELEGRAM WEBHOOK + BOT NOTIFY + EXCHANGE BALANCE v13
//
...
// memiliki autentikasi sama sekali — siapapun bisa memanggil endpoint
// ini tanpa token, termasuk trigger order atau overwrite API key user lain.
//
// Solusi: verifyBrowserAuth() mewajibkan header Authorization berisi
// Bearer <SUPABASE_ANON_KEY atau SERVICE_ROLE_KEY>.
// Caller dari browser harus sertakan header:
//   Authorization: Bearer <supabase_anon_key>
// Caller internal (cron/server) tetap pakai SERVICE_ROLE_KEY.
//
// Kenapa anon key cukup?
//   - Anon key hanya membuktikan request berasal dari app Z-Wealth
//     (bukan request liar dari luar).
//   - Validasi user_id per-request sudah dilakukan di DB query
//     (hanya data milik user tersebut yang bisa diakses).
//   - Untuk keamanan lebih tinggi, ganti dengan JWT validation
//     menggunakan sb.auth.getUser(token).
// ════════════════════════════════════════════════════════════════
const SUPABASE_ANON_KEY = Deno.env.get("SUPABASE_ANON_KEY") || "";

function verifyBrowserAuth(req: Request): boolean {
  const auth = req.headers.get("Authorization") || "";
  // Terima: service role key (internal) atau anon key (browser app)
  if (auth === `Bearer ${SUPABASE_SERVICE_KEY}`) return true;
  if (SUPABASE_ANON_KEY && auth === `Bearer ${SUPABASE_ANON_KEY}`) return true;
  // FIXED: Jangan terima request tanpa token sama sekali
  return false;
}
...
Deno.serve(async (req) => {
  ...
  if (req.method !== "POST") {
    return new Response(
      JSON.stringify({ status: "ok", service: "z-wealth trading worker v13" }),
      { status: 200, headers: { ...CORS_HEADERS, "Content-Type": "application/json" } }
    );
  }
