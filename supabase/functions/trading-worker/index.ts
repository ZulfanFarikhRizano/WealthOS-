// ══════════════════════════════════════════════════════════════════════
// Z-WEALTH · TRADING WORKER + TELEGRAM WEBHOOK + BOT NOTIFY + EXCHANGE BALANCE v19.8
//
// ── CHANGELOG v19.8 ─────────────────────────────────────────────────
...
// FIXED BUG-GHOST-POSITION (v19.7): Posisi aktif di exchange tapi tidak ada di DB
...
async function processUser(sb: SB, cfg: BotConfig): Promise<string[]> {
  ...
  // Guard leverage vs SL: pastikan leverage tidak terlalu tinggi relatif terhadap SL
  // Rasio aman: leverage × SL_PCT < 0.8 (artinya SL masih 20% sebelum titik likuidasi)
  const MAX_SAFE_LEVERAGE = Math.floor(0.8 / slPctForRisk); // 20x untuk SL 4%, 40x untuk SL 2%
  const lev = Math.min(userLev, MAX_SAFE_LEVERAGE);
  if (lev < userLev) {
    console.warn(`[processUser] Leverage dikurangi ${userLev}x → ${lev}x karena melebihi batas aman (SL=${slPctForRisk*100}%)`);
  }
