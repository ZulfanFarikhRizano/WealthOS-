// ══════════════════════════════════════════════════════════════════
// Z-WEALTH · TRADING WORKER + TELEGRAM WEBHOOK + BOT NOTIFY + EXCHANGE BALANCE v8-FINAL
//
...
const ADX_PERIOD       = 14;
const ADX_MIN_STRENGTH = 20;  // Skip entry jika ADX < 20 (sidewalk/no trend)
...
function isHTFBullish(htfCandles: Candle[]): boolean {
  ...
  return ema13Arr[n] > ema21Arr[n] && ema13Arr[n-1] > ema21Arr[n-1];
}
...
async function dbOpenPosition(
  ...
  await sb.from("positions").insert({
    ...
    peak_price: entryPrice, // trailing stop: mulai dari harga entry
...
async function handleExecuteOrder(body: any): Promise<Response> {
  ...
  // PATCH-2a: validasi side secara eksplisit — jangan biarkan typo masuk closeOrder
  ...
  try {
    // Tidak perlu fetchCandles — fetchRealtimePrice sudah punya fallback candle 1m internal.
    const realtimePrice = await fetchRealtimePrice(exchange, symbol, mode);
    // PATCH-2b: Jika harga tidak bisa diperoleh sama sekali, return error ke browser.
    // Fallback ke harga 1 berbahaya — qty yang dihitung akan salah total (misal BTC = $100k).
    if (realtimePrice <= 0) {
      return new Response(
...
Deno.serve(async (req) => {
  ...
  if (req.method !== "POST") {
    return new Response(
      JSON.stringify({ status: "ok", service: "z-wealth trading worker v8-final" }),
      { status: 200, headers: { ...CORS_HEADERS, "Content-Type": "application/json" } }
    );
  }
