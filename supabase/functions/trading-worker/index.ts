...
function tgShortCloseMsg(
  ...
  const pnlSign    = pnl >= 0 ? "+" : "";
  const emoji      = pnl >= 0 ? "💰" : "💸";
  // Short PnL% = (entry - exit) / entry × leverage × 100
  const pnlPct     = ((entryPrice - exitPrice) / entryPrice * 100 * leverage);
...
async function processUser(sb: SB, cfg: BotConfig): Promise<string[]> {
  ...
  const openSymbols    = new Set(currentPositions.map(p => p.symbol)); // unused guard, kept for safety
  ...
  // Kandidat: simbol yang belum punya posisi di kedua arah sekaligus
