// ══════════════════════════════════════════════════════════════════
// Z-WEALTH · TRADING WORKER + TELEGRAM WEBHOOK + BOT NOTIFY + EXCHANGE BALANCE v19.1
//
...
async function verifyUserOwnership(sb: SB, req: Request, userId: string): Promise<boolean> {
  ...
  // Frontend ideal: kirim supabase.auth.getSession().data.session?.access_token
  console.warn(`[verifyUserOwnership] Anon key fallback — DB ownership check untuk user_id=${userId.slice(0,8)}`);
  try {
