...
async function fetchBybitBalance(apiKey: string, apiSecret: string, mode: string): Promise<number> {
  const ts          = Date.now().toString();
  const recvWindow  = "5000";
  const accountType = mode === "futures" ? "CONTRACT" : "SPOT";
  ...
  if (!usdt && acct?.coin?.length) {
    usdt = acct.coin.reduce(
      (s: number, c: { usdValue?: string; walletBalance?: string }) =>
        s + parseFloat(c.usdValue || c.walletBalance || "0"),
      0
    );
  }
  return usdt;
