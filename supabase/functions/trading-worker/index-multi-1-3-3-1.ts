// ══════════════════════════════════════════════════════════════════
// Z-WEALTH · TRADING WORKER + TELEGRAM WEBHOOK + BOT NOTIFY + EXCHANGE BALANCE v18
//
...
async function bybitSetNativeTrailingStop(
  ...
  const body = JSON.stringify({
    ...
    // activePrice: kosong → trailing aktif langsung dari harga entry
  });
  const hdrs = await bybitHeaders(apiKey, apiSecret, body);
  ...
  return { orderId: "", ok: false, error: `NativeTS: ${data.retMsg} (trailStr=${trailStr})` };
}
...
async function openOrder(
  ...
  if (exchange === "bybit") {
    ...
    if (orderResult.ok && tradeMode === "futures") {
      const nativeTsResult = await bybitSetNativeTrailingStop(symbol, TRAILING_STOP_PCT_FUTURES, price, apiKey, apiSecret);
      if (!nativeTsResult.ok) {
        console.warn(`[openOrder] Native TS gagal di-set untuk ${symbol}: ${nativeTsResult.error} — software trailing tetap aktif`);
      } else {
        console.log(`[openOrder] Native trailing stop ${(TRAILING_STOP_PCT_FUTURES*100).toFixed(1)}% aktif di Bybit untuk ${symbol} (entryPrice=${price})`);
      }
    }
    return orderResult;
  } else {
...
async function openShortOrder(
  ...
  if (exchange === "bybit") {
    ...
    if (shortResult.ok) {
      const nativeTsResult = await bybitSetNativeTrailingStop(symbol, TRAILING_STOP_PCT_SHORT_FUTURES, price, apiKey, apiSecret);
      if (!nativeTsResult.ok) {
...
Deno.serve(async (req) => {
  ...
  if (req.method !== "POST") {
    return new Response(
      JSON.stringify({ status: "ok", service: "z-wealth trading worker v18.2" }),
      { status: 200, headers: { ...CORS_HEADERS, "Content-Type": "application/json" } }
    );
  }
