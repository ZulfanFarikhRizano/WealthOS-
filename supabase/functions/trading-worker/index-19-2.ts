// ══════════════════════════════════════════════════════════════════
// Z-WEALTH · TRADING WORKER + TELEGRAM WEBHOOK + BOT NOTIFY + EXCHANGE BALANCE v19
//
...
// FIXED BUG-AUTH-OWNERSHIP (v19 Kritis): Verifikasi bahwa JWT token pada request
// adalah milik user_id yang dikirim di body. Mencegah user A mengakses data user B
// hanya dengan mengirim user_id korban (sebelumnya hanya cek anon key = public).
//
// Flow:
//   1. Ambil token dari Authorization header
//   2. Jika token = service role key → bypass (cron/internal call) → return true
//   3. Jika token = anon key → TIDAK bisa verify ownership (anon = unauthenticated)
//      → return false (anon key tidak boleh dipakai untuk endpoint sensitif ownership)
//   4. Jika token = JWT user → panggil sb.auth.getUser(token) → cek uid === user_id
//
// Endpoint yang menggunakan fungsi ini:
//   - execute_order (close posisi finansial — kritis)
//   - exchange_balance (baca saldo — sensitif data)
//   - save_api_key (simpan API key — kritis)
//
// Endpoint yang TIDAK butuh ownership check:
//   - bot_status_notif: hanya kirim notif ke telegram_chat_id milik user sendiri
//   - telegram webhook: authenticated via Telegram platform (bukan supabase JWT)
...
async function verifyUserOwnership(sb: SB, req: Request, userId: string): Promise<boolean> {
  ...
  // Service role key: bypass ownership check (internal/cron call)
  if (token === SUPABASE_SERVICE_KEY) return true;

  // Anon key (public key): tidak bisa verifikasi kepemilikan → tolak untuk endpoint sensitif
  if (token === SUPABASE_ANON_KEY || token === _ANON_KEY_FALLBACK) {
    console.warn(`[verifyUserOwnership] Ditolak: anon key tidak bisa membuktikan ownership user_id=${userId.slice(0,8)}...`);
    return false;
  }

  // JWT user: verifikasi via Supabase Auth
  try {
    ...
    if (authUid !== userId) {
      console.warn(`[verifyUserOwnership] OWNERSHIP MISMATCH: token uid=${authUid.slice(0,8)}... ≠ claimed user_id=${userId.slice(0,8)}...`);
      return false;
    }
    return true;
  } catch (e) {
    console.error(`[verifyUserOwnership] Exception:`, e);
    return false;
  }
}
...
const ADX_MIN_STRENGTH = 22;  // Min ADX agar entry valid — trend harus cukup kuat
...
async function processUser(sb: SB, cfg: BotConfig): Promise<string[]> {
  ...
  if (entered === 0) {
    const now = Date.now();
    // FIXED BUG-SCAN-STATUS-PERSIST (v19 Medium): _scanStatusNotifAt sebelumnya
    // hanya in-memory. Setelah Edge Function cold start / restart, cooldown reset
    // → notif "bot aktif" terkirim lagi langsung meski baru dikirim beberapa menit lalu.
    // Ini inkonsisten dengan _lowBalanceNotifAt yang sudah persist ke bot_risk_state.
    //
    // FIX: Pola dua-lapis:
    //   L1 (in-memory): cek cepat tanpa DB round-trip (mayoritas tick)
    //   L2 (DB):        fallback saat in-memory kosong (cold start) — baca scan_status_notif_at
    //                   dari bot_risk_state, persist setelah notif dikirim
    //
    // Kolom DB yang diperlukan: bot_risk_state.scan_status_notif_at (timestamptz, nullable)
    ...
    if (lastNotifMemory === 0) {
      // Cold start: baca dari DB
      try {
        const { data: riskRow } = await sb
          .from("bot_risk_state")
          .select("scan_status_notif_at")
          .eq("user_id", userId)
          .maybeSingle();
        if (riskRow?.scan_status_notif_at) {
          lastNotif = new Date(riskRow.scan_status_notif_at).getTime();
          _scanStatusNotifAt.set(userId, lastNotif); // Isi L1 dari DB
        }
      } catch {
        // DB read gagal — lanjut dengan lastNotif=0 (boleh kirim notif)
      }
    }

    if (now - lastNotif >= SCAN_STATUS_INTERVAL_MS) {
      _scanStatusNotifAt.set(userId, now); // Update L1 segera
      // Persist ke DB (async, jangan block notif)
      sb.from("bot_risk_state")
        .upsert({ user_id: userId, scan_status_notif_at: new Date(now).toISOString() }, { onConflict: "user_id" })
        .then(({ error }) => { if (error) console.warn(`[processUser] Gagal persist scan_status_notif_at:`, error.message); })
        .catch(() => {}); // Silent — DB persist non-fatal
...
async function tgCmdHistory(sb: SB, chatId: number, userId: string): Promise<void> {
  try {
    const { data: trades } = await sb
      .from("trades")
      .select("symbol, side, price, amount_usdt, order_status, note, executed_at")
      .eq("user_id", userId)
