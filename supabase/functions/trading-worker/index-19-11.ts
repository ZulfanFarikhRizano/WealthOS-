// ══════════════════════════════════════════════════════════════════════
// Z-WEALTH · TRADING WORKER + TELEGRAM WEBHOOK + BOT NOTIFY + EXCHANGE BALANCE v19.11
//
// ── CHANGELOG v19.11 ────────────────────────────────────────────────
// STRATEGY AUDIT FIXES — 5 perbaikan strategi trading:
//
// FIX 1 — LOOKBACK 3→2 candle + EMA momentum check:
//   LOOKBACK 3 candle = sinyal bisa 45 menit stale → harga sudah jauh bergerak.
//   Dikurangi ke 2 candle (max 30 menit). Tambah cek momentum: spread EMA sekarang
//   harus masih ≥70% dari spread saat cross (toleransi 30% penyempitan).
//   Cross yang momentum-nya sudah habis = sinyal palsu → skip.
//
// FIX 2 — MAX_SAFE_LEVERAGE koef 0.8→0.3:
//   0.8 → MAX 40x untuk SL 2% → maks rugi 80% modal per trade (terlalu agresif).
//   0.3 → MAX 15x untuk SL 2% → maks rugi 30% modal per trade (wajar).
//   Jika leverage user > 2× batas aman → kirim notif Telegram warning.
//
// FIX 3 — closeOrder reject → deteksi likuidasi/manual close:
//   Jika Bybit reject close karena posisi tidak ada (error 110025/110045/position not found):
//   → Tandai posisi LIQUIDATED_OR_MANUAL di DB (bukan biarkan OPEN selamanya)
//   → Slot trading kembali terbuka
//   → Notif Telegram ke user
//   Jika error lain (network/auth) → tetap coba lagi di cron berikutnya.
//
// FIX 4 — Volume filter time-based (jam ramai vs sepi):
//   Fixed 1.2x → hampir tidak ada filter di jam ramai (07:00–23:00 WIB).
//   Sekarang: 1.5x di jam ramai (00:00–16:00 UTC), 1.2x di jam sepi (16:00–24:00 UTC).
//
// FIX 5 — HTF re-validasi saat exit (jangan exit prematur):
//   Sebelumnya: exit segera saat EMA 15m dead cross, meski HTF masih bullish.
//   Sekarang: jika HTF masih searah dengan posisi, abaikan EMA 15m cross exit.
//   SL dan Trailing Stop TETAP override (tidak terpengaruh HTF re-validasi).
//
// ── CHANGELOG v19.10 ────────────────────────────────────────────────
// FITUR BARU: S&R Filter — Pivot Point + Swing High/Low
//
//   Layer 1 — Pivot Point dari HTF candles (H4):
//     PP=((H+L+C)/3), R1,R2,S1,S2 dari candle sebelumnya yang sudah closed.
//   Layer 2 — Swing High/Low dari 50 candle 15m:
//     Swing High = lokal maxima (high > 5 candle kiri & kanan)
//     Swing Low  = lokal minima (low  < 5 candle kiri & kanan)
//
//   Decision (proximity = 2%):
//     LONG  → SKIP jika harga < 2% dari resistance (kecuali baru breakout)
//     SHORT → SKIP jika harga < 2% dari support    (kecuali baru breakdown)
//     Breakout/Breakdown → log SR_BREAKOUT_BONUS — sinyal terkuat
//
//   Notif Telegram sekarang include baris S&R:
//     📐 S&R: Res $0.0365 · Sup $0.0332 🚀 BREAKOUT!
//
// ── CHANGELOG v19.9 ─────────────────────────────────────────────────
// FITUR BARU: Balance History Snapshot — Grafik Saldo Bybit Real
//   Bot kini menyimpan snapshot saldo Bybit ke tabel balance_history
//   di akhir setiap cron run (non-sim). Throttle 10 menit per user.
//   Data ini digunakan oleh UI chart untuk menampilkan garis saldo
//   historis yang sesungguhnya (bukan rekonstruksi dari PnL trades).
//   Tabel baru: balance_history (user_id, balance_usdt, open_positions,
//               exchange, trade_mode, recorded_at)
//   SQL Migration:
...
//     );
//     CREATE INDEX ON balance_history (user_id, recorded_at DESC);
//
// ── CHANGELOG v19.8 ─────────────────────────────────────────────────
// FITUR BARU: Risk Management Mode — Auto & Manual
//
// LATAR BELAKANG:
//   Sebelumnya bot pakai leverage & trade_amount_usdt tetap dari settings.
//   Masalah: dengan leverage 20x dan SL 2%, kerugian = 40% modal per trade.
//   Jika SL_PCT tidak dimonitor (ghost position seperti kasus ZEREBROUSDT),
//   atau leverage terlalu tinggi, risiko MC sangat besar.
//
// SOLUSI: Dua mode risk management yang bisa dipilih di settings:
//
// MODE MANUAL (default, perilaku lama):
//   - Leverage dan trade_amount_usdt langsung dari settings user.
//   - Bot tidak mengubah amount berdasarkan saldo.
//   - Guard MAX_SAFE_LEVERAGE tetap aktif (cegah lev terlalu tinggi vs SL).
//
// MODE AUTO:
//   - User set leverage (manual) + risk_pct_per_trade (% saldo maks hilang per trade).
//   - Bot fetch saldo Bybit real-time SETIAP RUN sebelum entry.
//   - Bot hitung margin (amount) otomatis:
//       risk_usdt = saldo × (risk_pct / 100)
//       margin    = risk_usdt / (leverage × SL_PCT)
//   - Artinya: jika SL kena, kerugian SELALU = risk_pct% dari saldo, berapapun saldo saat ini.
//   - Contoh: saldo $100, risk_pct=1%, lev=10x, SL=2%:
//       risk_usdt = $1.00
//       margin    = $1.00 / (10 × 0.02) = $5.00
//       position  = $5 × 10x = $50 notional
//       loss jika SL = $50 × 2% = $1.00 ✅ (= 1% saldo)
//   - Jika fetch saldo gagal → fallback ke trade_amount_usdt dari settings (tidak block entry).
//   - Auto mode HANYA untuk futures — spot tidak ada leverage jadi formula berbeda.
//
// GUARD LEVERAGE (berlaku di kedua mode):
//   MAX_SAFE_LEVERAGE = floor(0.8 / SL_PCT)
//   Untuk SL 2%: max = 40x. Untuk SL 4%: max = 20x.
//   Jika leverage user melebihi batas aman → dikurangi otomatis + log warning.
//   Ini memastikan SL bot selalu trigger sebelum likuidasi exchange (buffer 20%).
//
// DB MIGRATION yang diperlukan (tambah kolom ke bot_configs):
//   ALTER TABLE bot_configs ADD COLUMN IF NOT EXISTS risk_mode TEXT DEFAULT 'manual';
//   ALTER TABLE bot_configs ADD COLUMN IF NOT EXISTS risk_pct_per_trade NUMERIC DEFAULT 1.0;
//
// ── CHANGELOG v19.8 ─────────────────────────────────────────────────
// ROOT CAUSE ANALYSIS — LIKUIDASI ZEREBROUSDT 05/05/2026 15:36 WIB:
//   Order LONG masuk ke Bybit (market, 1641 contracts, $0.036462).
//   Setelah order berhasil, kode lanjut ke getBybitInstrumentInfo() atau
//   dbOpenPosition() yang THROW exception (kemungkinan DB timeout/error).
//   Outer catch hanya: console.warn + log.push("ENTRY_EXCEPTION") + break.
//   Akibat: posisi HIDUP di Bybit tapi TIDAK ADA di DB → bot tidak tahu
//   posisi ini exist → SL & trailing stop tidak pernah jalan → posisi
//   dibiarkan terbuka selama ~40 menit → harga turun → kena LIKUIDASI.
//   Tidak ada notif Telegram karena tgSend belum sempat dipanggil.
//
// FIXED BUG-GHOST-POSITION-ENTRY (Kritis):
//   Entry block sebelumnya: 1 try/catch besar → openOrder + dbOpenPosition
//   + dbLogTrade + tgSend semua dalam 1 blok. Exception di mana saja
//   membuat tgSend tidak pernah dipanggil dan posisi bisa jadi ghost.
//
//   FIX: Entry flow di-refactor jadi 4 step dengan try/catch TERPISAH:
//
//   Step 1 — openOrder/openShortOrder:
//     Jika throw → outer catch menangkap → break (order belum masuk, aman).
//
//   Step 2 — dbOpenPosition (WAJIB BERHASIL):
//     Wrapped dalam try/catch sendiri. Jika GAGAL dan !sim:
//     → Langsung EMERGENCY CLOSE posisi di exchange (closeOrder/closeShortOrder)
//     → Kirim notif Telegram darurat dengan detail posisi & error
//     → Jika emergency close juga gagal → notif "🆘 CRITICAL: Ghost Position"
//        instruksikan user tutup manual di Bybit SEGERA
//     → break scan, jangan entry lagi
//
//   Step 3 — dbLogTrade:
//     try/catch SENDIRI, non-fatal. Error di sini hanya console.warn.
//     Posisi sudah aman di DB (Step 2 berhasil) — trade log adalah data sekunder.
//
//   Step 4 — tgSend notif entry:
//     try/catch SENDIRI, non-fatal. Error Telegram tidak boleh throw.
//     Notif adalah informasi sekunder — posisi & DB sudah aman di Step 2 & 3.
//
// FIXED BUG-GHOST-POSITION-RESCUE (Kritis):
//   syncPositionsWithExchange sebelumnya HANYA mengecek satu arah:
//   posisi di DB yang sudah tidak ada di exchange (→ close manual di exchange).
//   Tidak pernah mengecek arah sebaliknya:
//   posisi aktif di EXCHANGE yang tidak ada di DB (ghost position).
//   Early return "if (!dbPositions.length) return" membuat ghost position
//   tidak pernah terdeteksi saat DB kosong — kondisi paling berbahaya.
//
//   FIX A: Hapus early return — sync selalu cek exchange meski DB kosong.
//   FIX B: Tambah loop RESCUE setelah fetch posisi exchange:
//     - Bandingkan setiap posisi aktif di Bybit dengan set posisi di DB
//     - Jika ada di Bybit tapi TIDAK di DB → ghost position
//     - Rescue: dbOpenPosition() dengan data dari exchange (avgPrice, size, leverage)
//     - Bot langsung MONITORING posisi ini (SL & trailing stop aktif mulai cron berikutnya)
//     - Kirim notif Telegram "⚠️ Ghost Position Diselamatkan" dengan detail posisi
//     - Jika rescue gagal → notif "🚨 Ghost Position TIDAK BISA Diselamatkan"
//        instruksikan user tutup manual SEGERA
//
// ── CHANGELOG v19.6 ─────────────────────────────────────────────────
...
// Volume filter: volume candle saat cross harus > X× rata-rata 20 candle sebelumnya
// FIX v19.11: Time-based multiplier — jam ramai lebih ketat, jam sepi lebih longgar.
// Jam ramai (07:00–23:00 WIB / 00:00–16:00 UTC): multiplier 1.5 — banyak candle palsu.
// Jam sepi  (23:00–07:00 WIB / 16:00–00:00 UTC): multiplier 1.2 — volume memang rendah.
// Sebelumnya: fixed 1.2 → filter hampir tidak efektif di jam ramai.
function getVolumeMultiplier(): number {
  const hourUTC = new Date().getUTCHours();
  // Jam ramai = 00:00–16:00 UTC (07:00–23:00 WIB)
  return (hourUTC >= 0 && hourUTC < 16) ? 1.5 : 1.2;
}
const VOLUME_MULTIPLIER = 1.2; // fallback — gunakan getVolumeMultiplier() untuk dynamic
...
interface BotConfig {
  ...
  // ── RISK MANAGEMENT MODE (v19.11) ─────────────────────────────────
  // risk_mode: "auto" = bot auto-hitung leverage dari modal & risk_pct_per_trade
  //            "manual" = user set leverage manual, bot hitung margin dari balance
  // Jika kolom belum ada di DB (row lama), default ke "manual" (perilaku lama).
  risk_mode?:         "auto" | "manual";

  // risk_pct_per_trade: % dari total saldo yang boleh hilang per trade (saat SL kena).
  // Contoh: 1.0 = maksimal rugi 1% dari saldo per trade.
  // Hanya dipakai saat risk_mode = "auto". Range wajar: 0.5–3.0.
  // Jika tidak di-set, default 1.0%.
...
function checkVolume(candles: Candle[], crossIdx: number): { ok: boolean; ratio: number } {
  ...
  // FIX v19.11: Pakai time-based multiplier — lebih ketat di jam ramai
  const multiplier = getVolumeMultiplier();
  return { ok: ratio >= multiplier, ratio };
}
...
function detectCross(candles: Candle[]): SignalResult {
  ...
  // FIX v19.11: LOOKBACK dikurangi 3→2 candle.
  // 3 candle × 15 menit = sinyal bisa 45 menit stale → harga sudah jauh bergerak.
  // 2 candle = max 30 menit — masih actionable untuk entry.
  const LOOKBACK = 2;
  ...
  for (let i = 1; i <= LOOKBACK && n - i >= 0; i++) {
    ...
    // FIX v19.11: Tambah cek momentum — EMA13 dan EMA21 harus masih diverging (spread melebar).
    // Cross yang terjadi tapi EMA sudah menyempit kembali = momentum sudah habis = sinyal palsu.
    // Spread sekarang harus > spread saat cross (atau minimal sama).
    const spreadAtCross = Math.abs(cur13 - cur21);
    const spreadNow     = Math.abs(ema13Now - ema21Now);
    // Jika spreadAtCross = 0 (EMA tepat bersilangan), pakai threshold kecil (0.01% harga)
    // agar momentumOk tidak selalu true saat cross terjadi tepat di nol
    const minSpread  = (ema13Now * 0.0001); // 0.01% dari harga sebagai floor
    const effectiveSpreadAtCross = Math.max(spreadAtCross, minSpread);
    const momentumOk = spreadNow >= effectiveSpreadAtCross * 0.7; // toleransi 30% penyempitan

    if (prev13 < prev21 && cur13 >= cur21 && ema13Now >= ema21Now && momentumOk) {
      cross    = "GOLDEN";
      crossIdx = n - i + 1;
      break;
    }
...
// ══════════════════════════════════════════════════════════════════
// S&R DETECTION: Pivot Point (dari HTF candles) + Swing High/Low
// ══════════════════════════════════════════════════════════════════

interface SRLevels {
  pivotP:    number;  // Pivot Point (PP)
  pivotR1:   number;  // Resistance 1
  pivotR2:   number;  // Resistance 2
  pivotS1:   number;  // Support 1
  pivotS2:   number;  // Support 2
  swingHigh: number;  // Swing High terdekat dari 15m candles
  swingLow:  number;  // Swing Low terdekat dari 15m candles
}

function calcPivotPoints(htfCandles: Candle[]): Pick<SRLevels,"pivotP"|"pivotR1"|"pivotR2"|"pivotS1"|"pivotS2"> {
  if (!htfCandles || htfCandles.length < 2) return { pivotP:0, pivotR1:0, pivotR2:0, pivotS1:0, pivotS2:0 };
  const prev  = htfCandles[htfCandles.length - 2]; // candle HTF yang sudah closed
  const h = prev.high, l = prev.low, c = prev.close, range = h - l;
  const pp = (h + l + c) / 3;
  return { pivotP: pp, pivotR1: 2*pp - l, pivotR2: pp + range, pivotS1: 2*pp - h, pivotS2: pp - range };
}

function calcSwingLevels(candles: Candle[], lookback = 50, swing = 5): Pick<SRLevels,"swingHigh"|"swingLow"> {
  if (!candles || candles.length < swing * 2 + 1) return { swingHigh: 0, swingLow: 0 };
  const slice = candles.slice(-lookback);
  const swingHighs: number[] = [], swingLows: number[] = [];
  for (let i = swing; i < slice.length - swing; i++) {
    const c = slice[i];
    const lH = slice.slice(i-swing, i).map(x => x.high);
    const rH = slice.slice(i+1, i+swing+1).map(x => x.high);
    const lL = slice.slice(i-swing, i).map(x => x.low);
    const rL = slice.slice(i+1, i+swing+1).map(x => x.low);
    if (c.high > Math.max(...lH) && c.high > Math.max(...rH)) swingHighs.push(c.high);
    if (c.low  < Math.min(...lL) && c.low  < Math.min(...rL)) swingLows.push(c.low);
  }
  const cur = slice[slice.length - 1].close;
  const filteredHighs = swingHighs.filter(h => h > cur);
  const filteredLows  = swingLows.filter(l  => l < cur);
  const nearRes = filteredHighs.length > 0 ? filteredHighs.sort((a,b) => a-b)[0] : 0;
  const nearSup = filteredLows.length  > 0 ? filteredLows.sort((a,b)  => b-a)[0] : 0;
  return { swingHigh: nearRes, swingLow: nearSup };
}

function calcSRLevels(candles: Candle[], htfCandles: Candle[]): SRLevels {
  return { ...calcPivotPoints(htfCandles), ...calcSwingLevels(candles) };
}

// Proximity threshold: skip entry jika harga < 2% dari S&R level
const SR_PROXIMITY_PCT = 0.02;

interface SRCheckResult {
  allow:      boolean;
  reason:     string;
  isBreakout: boolean;  // harga baru breakout dari resistance → sinyal kuat
  nearestRes: number;
  nearestSup: number;
  distToRes:  number;   // % jarak ke resistance terdekat
  distToSup:  number;   // % jarak ke support terdekat
}

function checkSRFilter(price: number, sr: SRLevels, dir: "LONG"|"SHORT", prevCandle: Candle): SRCheckResult {
  const prevClose = prevCandle?.close || price;

  const resistances = [sr.pivotR1, sr.pivotR2, sr.swingHigh, sr.pivotP].filter(l => l > price && l > 0);
  const supports    = [sr.pivotS1, sr.pivotS2, sr.swingLow,  sr.pivotP].filter(l => l < price && l > 0);
  const nearestRes  = resistances.length > 0 ? resistances.reduce((a,b) => a < b ? a : b) : 0;
  const nearestSup  = supports.length    > 0 ? supports.reduce((a,b)    => a > b ? a : b) : 0;
  const distToRes   = nearestRes > 0 ? (nearestRes - price) / price : 1;
  const distToSup   = nearestSup > 0 ? (price - nearestSup) / price : 1;

  if (dir === "LONG") {
    // Breakout: candle sebelumnya di bawah resistance, sekarang di atas
    const broken = [sr.pivotR1, sr.pivotR2, sr.swingHigh].filter(l =>
      l > 0 && prevClose < l && price >= l * (1 - 0.003)
    );
    const isBreakout = broken.length > 0;

    // Skip jika terlalu dekat resistance KECUALI baru breakout
    if (nearestRes > 0 && distToRes < SR_PROXIMITY_PCT && !isBreakout) {
      return { allow: false, reason: `near_res:${(distToRes*100).toFixed(2)}%<2%_at=${nearestRes.toFixed(6)}`, isBreakout: false, nearestRes, nearestSup, distToRes, distToSup };
    }
    return { allow: true, reason: isBreakout ? `BREAKOUT_res=${broken.map(l=>l.toFixed(4)).join(",")}` : `ok_distRes=${(distToRes*100).toFixed(1)}%`, isBreakout, nearestRes, nearestSup, distToRes, distToSup };
  } else {
    // SHORT: Breakdown — candle sebelumnya di atas support, sekarang di bawah
    const broken = [sr.pivotS1, sr.pivotS2, sr.swingLow].filter(l =>
      l > 0 && prevClose > l && price <= l * (1 + 0.003)
    );
    const isBreakdown = broken.length > 0;

    if (nearestSup > 0 && distToSup < SR_PROXIMITY_PCT && !isBreakdown) {
      return { allow: false, reason: `near_sup:${(distToSup*100).toFixed(2)}%<2%_at=${nearestSup.toFixed(6)}`, isBreakout: false, nearestRes, nearestSup, distToRes, distToSup };
    }
    return { allow: true, reason: isBreakdown ? `BREAKDOWN_sup=${broken.map(l=>l.toFixed(4)).join(",")}` : `ok_distSup=${(distToSup*100).toFixed(1)}%`, isBreakout: isBreakdown, nearestRes, nearestSup, distToRes, distToSup };
  }
}
...
function tgBuyMsg(
  ...
  const resStr = srCheck && srCheck.nearestRes > 0 ? `$${srCheck.nearestRes.toLocaleString("en-US", { maximumFractionDigits: 6 })}` : "—";
  const supStr = srCheck && srCheck.nearestSup > 0 ? `$${srCheck.nearestSup.toLocaleString("en-US", { maximumFractionDigits: 6 })}` : "—";
  const srBreakout = srCheck?.isBreakout ? " 🚀 <b>BREAKOUT!</b>" : "";
  const srLine = srCheck ? `\n📐 S&R     : Res <code>${resStr}</code> · Sup <code>${supStr}</code>${srBreakout}` : "";
  return `🟢 <b>BUY ${symbol}</b>
━━━━━━━━━━━━━━━━━━━━
📊 Sinyal : <b>Golden Cross (${htfLabel})</b>
   EMA13 <code>${ema13.toFixed(4)}</code> ⬆ EMA21 <code>${ema21.toFixed(4)}</code>
📈 RSI    : <code>${rsi.toFixed(1)}</code>  ADX: <code>${adx.toFixed(1)}</code>${srLine}
...
function tgShortOpenMsg(
  ...
  const htfLabel       = htfConfirmed ? "15m ✓ 1H ✓" : "15m ✓ 1H –";
  const sResStr = srCheck && srCheck.nearestRes > 0 ? `$${srCheck.nearestRes.toLocaleString("en-US", { maximumFractionDigits: 6 })}` : "—";
  const sSupStr = srCheck && srCheck.nearestSup > 0 ? `$${srCheck.nearestSup.toLocaleString("en-US", { maximumFractionDigits: 6 })}` : "—";
  const sBreakdown = srCheck?.isBreakout ? " 🔻 <b>BREAKDOWN!</b>" : "";
  const sSrLine = srCheck ? `
📐 S&R     : Res <code>${sResStr}</code> · Sup <code>${sSupStr}</code>${sBreakdown}` : "";
  return `🔴 <b>SHORT ${symbol}</b>
━━━━━━━━━━━━━━━━━━━━
📊 Sinyal : <b>Dead Cross (${htfLabel})</b>
   EMA13 <code>${ema13.toFixed(4)}</code> ⬇ EMA21 <code>${ema21.toFixed(4)}</code>
📉 RSI    : <code>${rsi.toFixed(1)}</code>  ADX: <code>${adx.toFixed(1)}</code>${sSrLine}
💵 Harga  : <code>$${price.toLocaleString("en-US", { maximumFractionDigits: 6 })}</code>
🎯 Trail  : aktif setelah harga < <code>$${trailActPrice}</code>
...
// FIXED BUG-GHOST-POSITION (v19.8): Posisi aktif di exchange tapi tidak ada di DB
// → diselamatkan dengan membuat ulang record DB (rescue ghost position)
async function syncPositionsWithExchange(
  ...
  try {
    ...
    const exchangePositions: Record<string, string>[] = (data.result?.list || [])
    ...
    const activeOnExchange = new Set<string>(
      exchangePositions.map((p: Record<string, string>) => {
    ...
    const dbPositionKeys = new Set(dbPositions.map(p => {
      const dir: "LONG" | "SHORT" = p.direction === "SHORT" ? "SHORT" : "LONG";
      return `${p.symbol}:${dir}`;
    }));

    // ── RESCUE: Posisi aktif di exchange tapi TIDAK ADA di DB ────────
    // Ini adalah "ghost position" — ada di exchange, tidak dimonitor bot.
    // Tanpa monitoring: tidak ada SL, trailing stop, atau exit signal → RISIKO LIKUIDASI.
    // FIX: Buat ulang record DB dari data exchange sehingga bot mulai monitoring.
    for (const ep of exchangePositions) {
      const dir: "LONG" | "SHORT" = ep.side === "Buy" ? "LONG" : "SHORT";
      const key = `${ep.symbol}:${dir}`;
      if (!dbPositionKeys.has(key)) {
        const exchEntryPrice = parseFloat(ep.avgPrice || ep.entryPrice || "0");
        const exchQty        = parseFloat(ep.size || "0");
        const exchLev        = parseFloat(ep.leverage || "1");
        const exchAmtUsdt    = exchEntryPrice * exchQty / exchLev;

        if (exchEntryPrice <= 0 || exchQty <= 0) continue; // data tidak valid, skip

        console.warn(`[syncPositions] GHOST POSITION terdeteksi: ${ep.symbol} ${dir} — rescue ke DB`);
        try {
          await dbOpenPosition(sb, userId, ep.symbol, exchange, tradeMode,
            exchLev, exchEntryPrice, exchQty, exchAmtUsdt, false, dir);
          console.log(`[syncPositions] ${ep.symbol} ${dir} berhasil diselamatkan ke DB`);
          await tgSend(tgChat,
            `⚠️ <b>Ghost Position Diselamatkan</b>\n` +
            `━━━━━━━━━━━━━━━━━━━━\n` +
            `Posisi <b>${ep.symbol} ${dir}</b> ditemukan aktif di Bybit\n` +
            `tapi tidak ada di database bot.\n\n` +
            `📌 Simbol  : <b>${ep.symbol}</b>\n` +
            `📍 Arah    : <b>${dir}</b>\n` +
            `💵 Entry   : <code>$${exchEntryPrice.toLocaleString("en-US", { maximumFractionDigits: 6 })}</code>\n` +
            `📦 Qty     : <code>${exchQty}</code>\n` +
            `⚖️ Leverage: <code>${exchLev}x</code>\n\n` +
            `✅ Bot sekarang memantau posisi ini (SL & trailing aktif).\n` +
            `⏰ ${wib()}`
          );
        } catch (rescueErr) {
          console.error(`[syncPositions] Gagal rescue ghost position ${ep.symbol}:`, rescueErr);
          await tgSend(tgChat,
            `🚨 <b>Ghost Position TIDAK BISA Diselamatkan</b>\n` +
            `━━━━━━━━━━━━━━━━━━━━\n` +
            `<b>SEGERA cek & tutup manual: ${ep.symbol} ${dir} di Bybit!</b>\n` +
            `Bot tidak bisa memantau posisi ini.\n` +
            `⏰ ${wib()}`
          ).catch(() => {});
        }
      }
...
// ── Snapshot saldo Bybit ke balance_history ───────────────────────
// Dipanggil sekali tiap akhir cron run (non-sim, futures/spot).
// Throttle 10 menit per user agar tidak spam jika cron terlalu sering.
// Tabel balance_history: (user_id, balance_usdt, open_positions, exchange, trade_mode, recorded_at)
const _lastBalSnap = new Map<string, number>();
async function dbSnapshotBalance(
  sb: SB, userId: string, exchange: string, tradeMode: string,
  apiKey: string, apiSecret: string
): Promise<void> {
  if (Date.now() - (_lastBalSnap.get(userId) || 0) < 10 * 60 * 1000) return;
  try {
    const bal = exchange === "bybit"
    ...
    const { error } = await sb.from("balance_history").insert({
      user_id:        userId,
      balance_usdt:   parseFloat(bal.toFixed(4)),
      open_positions: openPos?.length || 0,
      exchange,
      trade_mode:     tradeMode,
      recorded_at:    new Date().toISOString(),
    });

    if (error) { console.warn(`[balSnap] ${error.message}`); return; }
    _lastBalSnap.set(userId, Date.now());
    console.log(`[balSnap] $${bal.toFixed(2)} USDT saved`);
  } catch (e) { console.warn(`[balSnap] non-fatal:`, e); }
}
...
async function dbClosePosition(
  ...
  await sb.from("positions").update({
    ...
    close_reason:   closeOrderId === "MANUAL_CLOSE" ? "MANUAL" : closeOrderId === "LIQUIDATED_OR_MANUAL" ? "LIQUIDATED" : "BOT_EXIT",
    pnl_usdt:       pnl,
...
async function processUser(sb: SB, cfg: BotConfig): Promise<string[]> {
  const {
    ...
    risk_pct_per_trade: riskPctRaw = 1.0,
  } = cfg;
  ...
  // ── RISK MANAGEMENT: Hitung amount & leverage berdasarkan risk_mode ──────
  // MODE MANUAL (default/lama): amount & leverage dari settings user, tidak berubah.
  // MODE AUTO  : bot fetch saldo real-time, lalu hitung leverage & margin yang tepat
  //              sehingga jika SL kena (-SL_PCT dari entry), kerugian = risk_pct_per_trade% dari saldo.
  //
  // Formula AUTO:
  //   risk_usdt    = saldo × (risk_pct / 100)          ← berapa USDT yang boleh hilang
  //   leverage_auto = risk_usdt / (margin × SL_PCT)    → tidak langsung, kita pakai:
  //   margin       = risk_usdt / SL_PCT                 ← margin yang dipakai (= amount futures)
  //   leverage_auto = clamp(floor(margin / margin), 1, MAX_SAFE_LEVERAGE)
  //
  //   Cara lebih mudah:
  //   Jika posisi futures dengan margin M dan leverage L:
  //     position_size = M × L
  //     pnl_saat_SL   = position_size × SL_PCT = M × L × SL_PCT
  //   Kita inginkan: M × L × SL_PCT = risk_usdt
  //   Kita kontrol M (margin/amount) dan L (leverage).
  //   Dengan L tetap (user pilih), M = risk_usdt / (L × SL_PCT).
  //   Dengan M tetap (trade_amount), L = risk_usdt / (M × SL_PCT).
  //
  //   Untuk AUTO mode: kita ambil L = user_leverage (dari DB), lalu hitung M otomatis.
  //   Ini berarti leverage tetap sesuai pilihan user, tapi amount per trade disesuaikan
  //   sehingga KERUGIAN MAKSIMAL selalu = risk_pct% dari saldo, bukan amount tetap.

  const slPctForRisk = tradeMode === "futures" ? SL_PCT_FUTURES : SL_PCT_SPOT;
  const riskPct      = Math.max(0.1, Math.min(10, riskPctRaw)); // clamp 0.1%–10%
  const userLev      = parseInt(String(leverage)) || 10;

  // Guard leverage vs SL:
  // FIX v19.11: Koef 0.8 → 0.3 — koef 0.8 menghasilkan MAX_SAFE_LEVERAGE=40x untuk SL 2%,
  // yang artinya loss per trade bisa mencapai 80% modal — jauh terlalu agresif.
  // Koef 0.3: MAX_SAFE_LEVERAGE = floor(0.3 / 0.02) = 15x untuk SL 2%.
  //   Dengan 15x dan SL 2%: maks rugi = 2% × 15 = 30% modal per trade. Lebih wajar.
  // Rumus: pastikan lev × SL_PCT ≤ 0.3 (maks rugi 30% modal per trade saat SL kena)
  const MAX_SAFE_LEVERAGE = Math.max(1, Math.floor(0.3 / slPctForRisk)); // min 1x, 15x untuk SL 2%
  const lev = Math.min(userLev, MAX_SAFE_LEVERAGE);
  if (lev < userLev) {
    console.warn(`[processUser] Leverage dikurangi ${userLev}x → ${lev}x — maks aman=${MAX_SAFE_LEVERAGE}x (SL=${slPctForRisk*100}% → max loss 30% modal)`);
    // Notif user jika leverage dikurangi drastis (> 2x perbedaan)
    if (userLev > MAX_SAFE_LEVERAGE * 2) {
      await tgSend(tgChat,
        `⚠️ <b>Leverage Dikurangi Otomatis</b>
` +
        `━━━━━━━━━━━━━━━━━━━━
` +
        `Setting kamu: <b>${userLev}x</b>
` +
        `Batas aman   : <b>${MAX_SAFE_LEVERAGE}x</b> (dengan SL ${(slPctForRisk*100).toFixed(0)}%)

` +
        `Bot pakai <b>${lev}x</b> untuk melindungi modal.
` +
        `Maks rugi per trade: <code>${(lev * slPctForRisk * 100).toFixed(0)}% dari margin</code>

` +
        `💡 <i>Ubah leverage di settings atau naikkan SL% untuk pakai leverage lebih tinggi.</i>
` +
        `⏰ ${wib()}`
      ).catch(() => {});
    }
  ...
  if (riskMode === "auto" && tradeMode === "futures" && !sim) {
    // AUTO MODE: fetch saldo real-time, hitung amount dari risk_pct
    try {
      const liveBal = exchange === "bybit"
        ? await fetchBybitBalance(apiKey, apiSecret, tradeMode)
        : await fetchBinanceBalance(apiKey, apiSecret, tradeMode);

      if (liveBal > 0) {
        // risk_usdt = berapa USDT yang boleh hilang jika SL kena
        const riskUsdt = liveBal * (riskPct / 100);
        // margin = berapa USDT yang dipakai sebagai margin (= amount di futures)
        // Karena: pnl_loss = margin × lev × SL_PCT → margin = riskUsdt / (lev × SL_PCT)
        const marginAuto = riskUsdt / (lev * slPctForRisk);
        amount = parseFloat(marginAuto.toFixed(2));
        console.log(
          `[processUser] AUTO RISK: saldo=$${liveBal.toFixed(2)} risk=${riskPct}%` +
          ` riskUsdt=$${riskUsdt.toFixed(2)} lev=${lev}x SL=${slPctForRisk*100}%` +
          ` → amount=$${amount.toFixed(2)} USDT`
        );
        log.push(`RISK_AUTO: bal=$${liveBal.toFixed(2)} risk=${riskPct}% → amount=$${amount.toFixed(2)} lev=${lev}x`);
      } else {
        // Saldo 0 atau gagal fetch → fallback ke manual amount
        amount = parseFloat(String(amountUsdt)) || 10;
        console.warn(`[processUser] AUTO RISK: saldo=0 atau fetch gagal → fallback amount=$${amount}`);
        log.push(`RISK_AUTO_FALLBACK: saldo=0 → amount=$${amount}`);
      }
    } catch (e) {
      // Fetch gagal → fallback ke manual amount, jangan block entry
      amount = parseFloat(String(amountUsdt)) || 10;
      console.warn(`[processUser] AUTO RISK: fetch saldo gagal (${e}) → fallback amount=$${amount}`);
      log.push(`RISK_AUTO_FALLBACK: fetchErr → amount=$${amount}`);
    }
  } else {
    // MANUAL MODE (default): pakai trade_amount_usdt dari settings langsung
    amount = parseFloat(String(amountUsdt)) || 10;
    if (riskMode === "auto" && tradeMode !== "futures") {
      // Auto mode hanya support futures — spot tidak ada leverage jadi perhitungan berbeda
      log.push(`RISK_AUTO_SPOT_SKIP: auto risk mode hanya support futures, pakai manual amount=$${amount}`);
    }
  }

  // DEBUG: log status sim agar kelihatan di Supabase logs
  console.log(`[processUser] userId=${userId.slice(0,8)} exchange=${exchange} sim=${sim} apiKeyLen=${apiKey?.length || 0} riskMode=${riskMode} lev=${lev} amount=${amount}`);
  ...
  for (const pos of openPositions) {
    ...
    try {
      const [candles, htfExitCandlesRaw] = await Promise.all([
        fetchCandles(exchange, pos.symbol, tradeMode),
        fetchHTFCandles(exchange, pos.symbol, tradeMode).catch(() => [] as Candle[]),
      ]);
      const htfExitCandles = htfExitCandlesRaw.length > 0 ? htfExitCandlesRaw : [];
      const { shouldExit: rawShouldExit, ema13, ema21 } = detectExitSignal(candles, direction);

      // FIX v19.11: Re-validasi HTF saat exit — jangan exit hanya karena EMA cross 15m
      // jika trend HTF masih kuat di arah yang sama.
      // SL dan Trailing Stop tetap override ini — hanya EMA cross exit yang di-filter.
      // Jika HTF candles gagal di-fetch (array kosong) → skip validasi, lanjut exit normal.
      let shouldExit = rawShouldExit;
      if (rawShouldExit && htfExitCandles.length >= 2) {
        const htfStillBullish = direction === "LONG"  && isHTFBullish(htfExitCandles);
        const htfStillBearish = direction === "SHORT" && isHTFBearish(htfExitCandles);
        if (htfStillBullish || htfStillBearish) {
          console.log(`[EXIT] ${pos.symbol} ${direction}: 15m cross tapi HTF masih searah — skip EMA exit`);
          shouldExit = false;
        }
      }
      ...
      if (!result.ok) {
        ...
        // FIX v19.11: Jika exchange reject close karena posisi sudah tidak ada
        // (sudah likuidasi atau di-close manual), update DB juga agar slot tidak permanen penuh.
        const alreadyClosed =
          errMsg.includes("position not found") ||
          errMsg.includes("no position") ||
          errMsg.includes("order not exist") ||
          errMsg.includes("position size is zero") ||
          errMsg.includes("110025") || // Bybit: position not exists
          errMsg.includes("110045");   // Bybit: position is closing

        if (alreadyClosed) {
          // Posisi sudah tidak ada di exchange — tutup di DB juga
          const liquidatedPnl = direction === "LONG"
            ? (exitPrice - pos.entry_price) / pos.entry_price * pos.leverage * pos.amount_usdt
            : (pos.entry_price - exitPrice) / pos.entry_price * pos.leverage * pos.amount_usdt;
          await dbClosePosition(sb, pos.id, exitPrice, "LIQUIDATED_OR_MANUAL", liquidatedPnl, sim).catch(() => {});
          await dbLogTrade(sb, userId, pos.symbol, direction === "LONG" ? "SELL" : "BUY",
            exitPrice, pos.amount_usdt, ema13, ema21, "", "LIQUIDATED", result.error || null,
            `CLOSE ${direction}: posisi sudah tidak ada di exchange (likuidasi/manual close)`);
          log.push(`${pos.symbol}:CLOSED_${direction}:LIQUIDATED_OR_MANUAL`);
          await tgSend(tgChat,
            `⚠️ <b>Posisi Tertutup di Exchange</b>
` +
            `━━━━━━━━━━━━━━━━━━━━
` +
            `<b>${direction} ${pos.symbol}</b> sudah tidak ada di Bybit
` +
            `(kemungkinan likuidasi atau di-close manual).

` +
            `📌 DB diperbarui otomatis — slot trading kembali terbuka.
` +
            `💵 Entry : <code>$${pos.entry_price.toLocaleString("en-US", { maximumFractionDigits: 6 })}</code>
` +
            `💵 Exit  : <code>$${exitPrice.toLocaleString("en-US", { maximumFractionDigits: 6 })}</code>
` +
          ...
          // Error lain (network, auth, dll) — jangan update DB, coba lagi di cron berikutnya
          await dbLogTrade(sb, userId, pos.symbol, direction === "LONG" ? "SELL" : "BUY",
  ...
  // NOTE: Di auto mode, saldo sudah di-fetch di atas untuk hitung amount.
  // Di sini kita tetap cek apakah amount hasil perhitungan masuk akal vs saldo.
  if (!sim) {
    try {
      const availBal = exchange === "bybit"
        ? await fetchBybitBalance(apiKey, apiSecret, tradeMode)
        : await fetchBinanceBalance(apiKey, apiSecret, tradeMode);
      const needed = tradeMode === "futures" ? amount : amount; // futures: margin = amount (tanpa leverage)
      console.log(`[processUser] balance=${availBal.toFixed(2)} needed=${needed.toFixed(2)} amount=${amount} mode=${riskMode}`);
  ...
  for (let i = 0; i < candidateSymbols.length && !(curLongFull && curShortFull); i += BATCH_SIZE) {
    ...
    const batchResults = await Promise.allSettled(
      batch.map(async (symbol) => {
        ...
        const sr          = calcSRLevels(candles, htfCandles);
        return { symbol, candles, signal, htfBullish, htfBearish, rsi, adx, sr };
      })
    );
    ...
    for (const res of batchResults) {
      ...
      const { symbol, candles, signal: { cross, ema13, ema21, volumeOk, volumeRatio }, htfBullish, htfBearish, rsi, adx, sr } = res.value;
      ...
      // ── Filter S&R ───────────────────────────────────────────────
      // Pivot Point (dari HTF) + Swing High/Low (dari 15m candles)
      // Skip jika harga terlalu dekat resistance (LONG) / support (SHORT)
      // KECUALI harga baru breakout — justru konfirmasi momentum kuat
      // Fetch harga real-time SEKALI — dipakai untuk SR check DAN entry price
      // (menghindari double fetch ke exchange per simbol)
      const realtimeForSR  = await fetchRealtimePrice(exchange, symbol, tradeMode).catch(() => 0);
      const srPrice        = realtimeForSR > 0 ? realtimeForSR : candles[candles.length - 1].close;
      const prevCandle     = candles[candles.length - 2] || candles[candles.length - 1];
      const srCheck        = checkSRFilter(srPrice, sr, entryDirection, prevCandle);
      // Simpan srPrice untuk dipakai sebagai entryPrice (reuse, tidak fetch ulang)
      const cachedRealtimePrice = realtimeForSR;

      console.log(
        `[SR] ${symbol} ${entryDirection}: ${srCheck.reason} ` +
        `res=${srCheck.nearestRes.toFixed(6)} sup=${srCheck.nearestSup.toFixed(6)} ` +
        `distRes=${(srCheck.distToRes*100).toFixed(2)}% distSup=${(srCheck.distToSup*100).toFixed(2)}% ` +
        `breakout=${srCheck.isBreakout}`
      );

      if (!srCheck.allow) {
        log.push(`${symbol}:SKIP_SR:${srCheck.reason}`);
        continue;
      }
      if (srCheck.isBreakout) {
        log.push(`${symbol}:SR_BREAKOUT_BONUS:${srCheck.reason}`);
      }
      ...
      try {
        // Reuse harga dari SR check (sudah di-fetch di atas) — tidak perlu fetch ulang
        const entryPrice    = cachedRealtimePrice > 0 ? cachedRealtimePrice : candles[candles.length - 1].close;
        ...
        // ── ORDER SUDAH MASUK KE EXCHANGE ─────────────────────────────
        // KRITIS: Dari titik ini, order sudah tereksekusi di Bybit/Binance.
        // Setiap operasi selanjutnya (DB, Telegram) harus di-wrap try/catch terpisah
        // agar exception tidak membuat posisi jadi "ghost" (ada di exchange, tidak ada di DB).
        // Ghost position = tidak ada SL/trailing monitoring = RISIKO LIKUIDASI.
        ...
        // FIXED BUG-GHOST-POSITION (Kritis): dbOpenPosition bisa throw jika DB timeout/error.
        // Sebelumnya: exception langsung ke outer catch → break tanpa DB tersimpan → ghost position.
        // FIX: Coba simpan ke DB, jika GAGAL → EMERGENCY CLOSE order di exchange sekarang juga
        // agar posisi tidak terbuka tanpa monitoring (mencegah likuidasi seperti kasus ZEREBROUSDT).
        let dbSaved = false;
        try {
          await dbOpenPosition(sb, userId, symbol, exchange, tradeMode, lev, entryPrice, actualQty, amount, sim, entryDirection);
          dbSaved = true;
        } catch (dbErr: unknown) {
          const dbErrStr = String(dbErr).slice(0, 120);
          console.error(`[Worker] KRITIS: dbOpenPosition GAGAL setelah order masuk — ${symbol}: ${dbErrStr}`);
          log.push(`${symbol}:GHOST_POSITION_RISK:${dbErrStr}`);

          // EMERGENCY CLOSE: Tutup posisi di exchange sekarang juga agar tidak jadi ghost position
          if (!sim) {
            try {
              const emergencyResult = entryDirection === "LONG"
                ? await closeOrder(exchange, tradeMode, symbol, actualQty, apiKey, apiSecret)
                : await closeShortOrder(exchange, symbol, actualQty, apiKey, apiSecret);
              const emergencyStatus = emergencyResult.ok ? "BERHASIL" : `GAGAL: ${emergencyResult.error}`;
              console.error(`[Worker] EMERGENCY CLOSE ${symbol}: ${emergencyStatus}`);
              log.push(`${symbol}:EMERGENCY_CLOSE:${emergencyStatus}`);

              // Kirim notif darurat ke Telegram
              await tgSend(tgChat,
                `🚨 <b>EMERGENCY CLOSE — ${symbol}</b>\n` +
                `━━━━━━━━━━━━━━━━━━━━\n` +
                `Order masuk ke exchange tapi GAGAL disimpan ke database.\n` +
                `Bot melakukan close darurat untuk mencegah posisi tanpa monitoring.\n\n` +
                `📌 Simbol  : <b>${symbol}</b>\n` +
                `📍 Arah    : <b>${entryDirection}</b>\n` +
                `💵 Entry   : <code>$${entryPrice.toLocaleString("en-US", { maximumFractionDigits: 6 })}</code>\n` +
                `🔒 Close   : <b>${emergencyStatus}</b>\n` +
                `⚠️ Error DB: <code>${dbErrStr}</code>\n\n` +
                `<i>Periksa akun Bybit Anda untuk memastikan posisi sudah tertutup.</i>\n` +
                `⏰ ${wib()}`
              ).catch(() => {});
            } catch (emergencyErr) {
              console.error(`[Worker] EMERGENCY CLOSE juga GAGAL untuk ${symbol}:`, emergencyErr);
              // Notif terakhir — posisi mungkin masih terbuka tanpa monitoring
              await tgSend(tgChat,
                `🚨🚨 <b>KRITIS: Ghost Position ${symbol}</b>\n` +
                `━━━━━━━━━━━━━━━━━━━━\n` +
                `Order MASUK ke exchange, DB GAGAL simpan, Emergency Close GAGAL.\n` +
                `<b>SEGERA cek dan tutup posisi ${symbol} ${entryDirection} secara manual di Bybit!</b>\n` +
                `⏰ ${wib()}`
              ).catch(() => {});
            }
          }
          break; // stop scan — sudah ditangani
        }

        // DB berhasil tersimpan, lanjut log trade & notif
        // Wrap masing-masing agar tidak block satu sama lain
        try {
          await dbLogTrade(sb, userId, symbol, entryDirection === "LONG" ? "BUY" : "SELL",
          ...
          console.warn(`[Worker] dbLogTrade gagal (non-fatal) ${symbol}:`, logErr);
        }

        // FIXED: tgSend di-wrap terpisah — error Telegram tidak boleh membatalkan posisi yang sudah tersimpan
        try {
          if (entryDirection === "LONG") {
            await tgSend(tgChat, tgBuyMsg(symbol, entryPrice, actualQty, amount, tradeMode, lev, ema13, ema21, sim, htfBullish, rsi, adx, srCheck));
          } else {
            await tgSend(tgChat, tgShortOpenMsg(symbol, entryPrice, actualQty, amount, lev, ema13, ema21, sim, htfBearish, rsi, adx, srCheck));
          }
        } catch (tgErr) {
          console.warn(`[Worker] tgSend gagal (non-fatal) setelah entry ${symbol}:`, tgErr);
        }

        if (!dbSaved) break; // safety guard (seharusnya tidak tercapai)
        ...
        // Break eksplisit — tidak ada entry kedua dalam 1 run
        break;
      } catch (e: unknown) {
        // Catch ini hanya untuk error SEBELUM order dikirim ke exchange
        // (fetchRealtimePrice, openOrder/openShortOrder gagal)
        // Setelah order masuk, semua error ditangani di blok terpisah di atas.
        console.warn(`[Worker] Entry error pre-order ${symbol}:`, e);
  ...
  // ── FASE AKHIR: Snapshot saldo ke DB untuk grafik historis ─────
  // Non-fatal, tidak block return. Throttle 10 menit di dalam fungsi.
  if (!sim) {
    dbSnapshotBalance(sb, userId, exchange, tradeMode, apiKey, apiSecret).catch(e =>
      console.warn("[processUser] balSnap error (ignored):", e)
    );
  }
