...
async function processUser(sb: SB, cfg: BotConfig): Promise<string[]> {
  ...
  // FIXED: Coba berbagai nama field api_key yang mungkin ada di DB
  // Supabase AI log menunjukkan bot masih SIMULATED — kemungkinan nama kolom berbeda
  const cfgAny = cfg as Record<string, string>;
  const apiKey    = cfg.api_key || cfgAny["bybit_api_key"] || cfgAny["apiKey"] || cfgAny["api_key_bybit"] || "";
  const apiSecret = cfg.api_secret || cfgAny["bybit_api_secret"] || cfgAny["apiSecret"] || cfgAny["api_secret_bybit"] || "";
