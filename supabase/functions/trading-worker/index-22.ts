...
function isServerEncrypted(s: string): boolean {
  ...
  const [iv, ct] = s.split(":");
  // IV hex = 24 karakter (12 bytes), CT minimal ada isinya
  return iv.length === 24 && ct.length > 0 && /^[0-9a-f]+$/i.test(iv);
}
...
function detectCross(candles: Candle[]): SignalResult {
  ...
  // FIXED: Lookback 3 candle terakhir agar sinyal tidak terlewat
  // Sebelumnya hanya cek candle n vs n-1 → miss kalau cross terjadi 2-3 candle lalu
  // Sekarang: cek apakah ada cross dalam 3 candle terakhir DAN posisi EMA saat ini masih valid
  const LOOKBACK = 3;
  ...
  for (let i = 1; i <= LOOKBACK && n - i >= 0; i++) {
    ...
    if (prev13 < prev21 && cur13 >= cur21) {
      // Golden Cross terjadi dalam lookback window
      // Konfirmasi: EMA13 masih di atas EMA21 saat ini (trend masih valid)
      if (ema13Now >= ema21Now) { cross = "GOLDEN"; break; }
    }
    if (prev13 > prev21 && cur13 <= cur21) {
      // Dead Cross terjadi dalam lookback window
      if (ema13Now <= ema21Now) { cross = "DEAD"; break; }
    }
  }
...
function detectExitSignal(candles: Candle[]): ExitSignalResult {
  ...
  const ema13Now = ema13Arr[n];
  const ema21Now = ema21Arr[n];
  // Exit saat EMA13 < EMA21 (dead cross sudah terjadi & berlanjut)
  return { shouldExit: ema13Now < ema21Now, ema13: ema13Now, ema21: ema21Now };
}
...
function tgSellMsg(
  ...
  const signalLabel = reason === "STOP_LOSS" ? "🛑 Stop Loss (>5%)" : "📊 Dead Cross";
...
async function syncPositionsWithExchange(
  ...
  // syncPositions hanya support Bybit untuk saat ini
  ...
  try {
    ...
    for (const pos of dbPositions) {
      if (!activeOnExchange.has(pos.symbol)) {
        ...
        // Ambil harga terakhir sebagai exit price estimasi
        const candles  = await fetchCandles(exchange, pos.symbol, tradeMode);
        const exitPrice = candles[candles.length - 1]?.close || pos.entry_price;
...
async function dbLogTrade(
  ...
  // FIXED: Normalisasi status agar konsisten dengan yang dibaca app.js
  // App.js menghitung stats dari: FILLED, SIMULATED, CLOSED
  // Edge Function dulu nulis: OPEN, SIM_OPEN, CLOSED, SIM_CLOSED → tidak terbaca stats
...
async function processUser(sb: SB, cfg: BotConfig): Promise<string[]> {
  ...
  console.log(`[processUser] userId=${userId.slice(0,8)} exchange=${exchange} sim=${sim} apiKeyLen=${apiKey?.length || 0} apiKeyPreview=${apiKey?.slice(0,6) || "EMPTY"} allKeys=${Object.keys(cfg).join(",")}`);
  ...
  for (const pos of openPositions) {
    ...
    try {
      ...
      // Stop Loss otomatis: harga turun >5% dari entry (futures only)
      const currentPrice  = candles[candles.length - 1].close;
      const priceDrop     = pos.entry_price > 0 ? (pos.entry_price - currentPrice) / pos.entry_price : 0;
      const stopLossHit   = tradeMode === "futures" && priceDrop > 0.05;

      if (!shouldExit && !stopLossHit) continue;

      const exitReason = stopLossHit ? "STOP_LOSS" : "DEAD_CROSS";
  ...
  if (!sim) {
    try {
      ...
      const needed = amount * (tradeMode === "futures" ? 1 : 1); // margin needed (approx)
