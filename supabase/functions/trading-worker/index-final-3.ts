// ══════════════════════════════════════════════════════════════════
// Z-WEALTH · TRADING WORKER + TELEGRAM WEBHOOK + BOT NOTIFY + EXCHANGE BALANCE v11-FINAL
//
...
function isStablecoin(symbol: string): boolean {
  ...
  // Simbol yang harganya hampir persis $1 (±2%) = stablecoin
  // Ini dicek saat fetch ticker, tapi sebagai pre-filter:
  // Nama yang diawali "USD" hampir pasti stablecoin
  if (/^USD[A-Z0-9]+USDT$/.test(symbol)) return true;
  // Simbol yang diakhiri dengan huruf yang menunjuk fiat pegged
...
async function processUser(sb: SB, cfg: BotConfig): Promise<string[]> {
  ...
  console.log(`[processUser] scan: ${symbols.length} simbol, slots=${slots}, openPos=${openCount}, sim=${sim}, exchange=${exchange}, mode=${tradeMode}`);
  let entered   = 0;
    ...
    for (const res of batchResults) {
      ...
      try {
        ...
        if (!result.ok) {
          ...
          console.error(`[ENTRY_ERR] ${exchange} ${symbol}: ${errMsg}`);
...
Deno.serve(async (req) => {
  ...
  if (req.method !== "POST") {
    return new Response(
      JSON.stringify({ status: "ok", service: "z-wealth trading worker v11-final" }),
      { status: 200, headers: { ...CORS_HEADERS, "Content-Type": "application/json" } }
    );
  }
