...
async function handleTelegramWebhook(sb: SB, body: any): Promise<Response> {
  ...
  // ── Handle bot_status_notif dari browser (proxy kirim pesan) ─────
  if (body?._type === "bot_status_notif") {
    const chatId  = body.chat_id;
    const message = body.message;
    if (chatId && message) await tgSend(chatId, message);
  ...
  // ── Cara 2: fallback — cari via telegram_chat_id yang sudah tersimpan ──
  if (!userId) {
  ...
  // ── User tidak ditemukan — tetap balas dengan panduan ────────────
  if (!userId) {
    await tgSend(chatId,
...
Deno.serve(async (req) => {
  ...
  // ── Selalu parse body dulu jika POST ─────────────────────────────
  if (req.method === "POST") {
    ...
    // ── CEK TELEGRAM WEBHOOK PERTAMA — sebelum auth check apapun ────
    // Telegram selalu kirim field "update_id" di setiap update
    if ("update_id" in body) {
      return await handleTelegramWebhook(sb, body);
    }

    // ── Bukan Telegram → Cron Job, perlu auth ────────────────────────
